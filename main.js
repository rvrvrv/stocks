...
$(document).ready(() => {
  ...
  socket.on('added', data => addStock(data.addedStockData));
...
function addStock(stockObj) {
  ...
  const stock = Object.keys(stockObj)[0];
...
$('.modals').on('click', '.del-btn', function () {
  ...
  removeStock(stock);
  /* Remove stock from list on server, which also updates all other clients */
